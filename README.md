...
│   ├── motor.py                       # Motor de consultas, DuckDB y Google Gemini RAG
...
* **Panel Lateral (Sidebar):** Ingesta de archivos con detección automática de tipo, explorador con filtro por categoría, acciones rápidas (`Reindexar`, `Limpiar Chat`), **Bóveda de Credenciales `[VAULT]`** con inputs protegidos y telemetría de motores en tiempo real.
* **Pestaña 1 (Consultas y Búsqueda):** Consultas técnicas en lenguaje natural potenciadas por **Google Gemini RAG** (`gemini-3.6-flash`) cuando la clave está en la bóveda, o por el motor local autónomo (DuckDB + MarkItDown) con conmutación transparente ante fallos de red.
* **Pestaña 2 (Historial de Mantenimientos):** Tabla interactiva de servidores con filtros por Nivel de Arquitectura (L1-L4), Estado y Técnico, más consola SQL DuckDB en vivo sobre `mantenimientos.csv`.
...
| **Motor SQL en Memoria** | DuckDB + Pandas | Consultas ultrarrápidas sobre mantenimientos e inventarios CMDB |
