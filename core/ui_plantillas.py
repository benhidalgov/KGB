...
def renderizar_pestana_plantillas(doc_store: dict):
    ...
    with col_t1:
        st.markdown("#### 1. Datos del Procedimiento")
        tipo_sel = st.selectbox("Tipo de Procedimiento", obtener_todos_los_tipos_plantillas(), key="select_tipo_procedimiento_gen")
        es_nuevo_t = "[+ Crear" in tipo_sel

        if es_nuevo_t:
            nuevo_t_nom = st.text_input("Nombre del Tipo (*)", key="input_nuevo_tipo_proc")
            guardar_cat = st.checkbox("Guardar para usarlo de nuevo", value=True)
        ...
        with st.expander("Ver plantillas de ejemplo (Opcional)", expanded=False):
            col_rs, col_rb = st.columns([3, 1])
            with col_rs:
                base_act = st.selectbox("Elegir plantilla de ejemplo:", PLANTILLAS_BASE_RESERVADAS, key="sel_plantilla_base_res")
            with col_rb:
                st.write("")
                st.write("")
                if st.button("[+ Usar]", key="btn_activar_plantilla_base", width="stretch"):
                    guardar_plantilla_personalizada(base_act, f"Plantilla activada: {base_act}", ["criterio", "pasos", "verif"])
                    st.toast(f"Plantilla '{base_act}' activada.")
                    st.rerun()
        ...
        with col_g3:
            criticidad = st.selectbox("Criticidad", ["Crítico 7x24 (P1)", "Alta (P2)", "Media (P3)", "Baja (P4)"], index=2, key="proc_crit_input")
        with col_g4:
        ...
        st.markdown("---")
        st.markdown("##### Detalles del Procedimiento")
    ...
    with col_t2:
        ...
        with col_rb_c1:
            cat_rb_exist = st.selectbox("Categoría (*):", ["(Crear Nueva)"] + cats_disp_rb, key="sb_cat_proc_exist") if cats_disp_rb else "(Crear Nueva)"
        with col_rb_c2:
            cat_rb_nueva = st.text_input("Nueva Categoría (*):" if cat_rb_exist == "(Crear Nueva)" else "O escribir otra categoría:", key="input_cat_proc_nueva")

        with st.container(border=True):
            st.markdown(doc_gen_md)

        st.divider()
        if st.button("Guardar Documento", type="primary", width="stretch", key="btn_guardar_doc_plantilla_final"):
